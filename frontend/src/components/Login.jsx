...
const Login = ({ onLogin }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setIsLoading(true);
    const userPool = new CognitoUserPool(cognitoConfig); // Sử dụng trực tiếp cognitoConfig
  ...
  const handleGoogleLogin = () => {
    const url = `https://ap-southeast-2t30owwizg.auth.ap-southeast-2.amazoncognito.com/login?response_type=code&client_id=${cognitoConfig.ClientId}&redirect_uri=${encodeURIComponent(cognitoConfig.RedirectUri)}&scope=${encodeURIComponent(cognitoConfig.Scope || 'openid+email+profile')}`;
        ...
        try {
            const response = await fetch('https://ap-southeast-2t30owwizg.auth.ap-southeast-2.amazoncognito.com/oauth2/token', {
                ...
                body: new URLSearchParams({
                    ...
                    client_secret: cognitoConfig.ClientSecret || '', // Thêm nếu có
                    code,
            ...
            if (response.ok) {
                if (data.id_token) {
                    ...
                    setErrors({ general: 'Không tìm thấy id_token trong response: ' + JSON.stringify(data) });
                }
            } else {
                setErrors({ general: `Lỗi từ server: ${data.error} - ${data.error_description || 'Không có mô tả'}` });
            }
        } catch (err) {
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl p-10 border border-white/20">
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div>
                ...
                {errors.general && <p className="text-red-400 text-sm mt-1">{errors.general}</p>}
              </div>
