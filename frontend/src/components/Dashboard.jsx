...
const Dashboard = ({ user, onLogout }) => {
  ...
  // Fetch mindmaps from backend when component mounts or token changes
  useEffect(() => {
    const token = localStorage.getItem('cognito_token');
    if (token) {
      fetch('http://localhost:8081/api/mindmap', {
        headers: { Authorization: `Bearer ${token}` },
          ...
          .catch(error => {
            ...
            showNotificationMessage('Lỗi khi tải mindmap!', 'error');
          });
    }
    ...
    if (newName && newName.trim()) {
      setMindmaps(prev => prev.map(m =>
          m.id === currentMindmap.id
              ? { ...m, name: newName.trim(), updated: new Date().toISOString().split("T")[0] }
              : m
      ));
  ...
  // Save current mindmap (gửi lên backend nếu cần)
  const saveMindmap = () => {
    if (!currentMindmap) return;
    const token = localStorage.getItem('cognito_token');
    if (token) {
      fetch(`http://localhost:8080/api/mindmap/${currentMindmap.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ ...currentMindmap, updated: new Date().toISOString().split("T")[0] }),
      })
          .then(response => {
          ...
          .then(() => {
            setMindmaps(prev => prev.map(m =>
                m.id === currentMindmap.id ? { ...m, updated: new Date().toISOString().split("T")[0] } : m
            ));
            showNotificationMessage("Đã lưu thành công!", "success");
          })
          .catch(error => {
            console.error('Error saving mindmap:', error);
            showNotificationMessage('Lỗi khi lưu mindmap!', 'error');
          });
    }
  };
  ...
  const deleteMindmap = (id) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa mindmap này?")) {
      const token = localStorage.getItem('cognito_token');
      if (token) {
        fetch(`http://localhost:8080/api/mindmap/${id}`, {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        })
            .then(response => {
            ...
            .catch(error => {
              ...
              showNotificationMessage('Lỗi khi xóa mindmap!', 'error');
            });
      }
    }
  };
